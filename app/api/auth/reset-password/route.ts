...
export async function POST(request: Request) {
  ...
  if (!token || password.length < 8 || password.length > 128) return Response.json({ message: "Use a valid reset link and a password between 8 and 128 characters." }, { status: 400 });
