...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await supabaseService().rpc("karaoke_v2_update_transcription_job", {
    target_job_id: id,
    next_status: status,
    next_progress: progress,
    failure_message: status === "failed" ? String(body.error || "Transcription failed.").slice(0, 2000) : null,
  });
