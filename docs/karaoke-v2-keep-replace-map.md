...
| Auth/admin UI | `lib/stagefront-auth.ts`, auth API routes, and `app/admin` provide reusable application identity/admin patterns. | Keep and reuse auth/admin primitives; add a separate v2 UI later. | All project/job routes must enforce owner or admin access. Do not alter current admin pages during foundation work. |
...
This foundation creates no database objects, storage buckets, workers, uploads, navigation links, or changes to existing routes. It therefore cannot intercept or alter current application behavior.
